...
window.addEventListener('load', () => {
  ...
  $('#isComplete').addEventListener('click', (e) => {
    $('form>button').innerText = e.target.checked ? 'Simpan Sebagai Sudah Dibaca' : 'Simpan Sebagai Belum Dibaca';
  });
});


function updateUI(books) {
  const finished = $('.sudah-dibaca'), 
  ...
  for(let book of books) {
    ...
    let group = $new('button', { css: ['button', 'button-success'], text: `${ book.isComplete ? 'Belum Selesai Dibaca' : 'Sudah Selesai Dibaca'}`, action: moveGroup });
  ...
  !finished.hasChildNodes() ? finished.innerHTML = 'Data Kosong' : 0;
  !unfinished.hasChildNodes() ? unfinished.innerHTML = 'Data Kosong' : 0;
}
